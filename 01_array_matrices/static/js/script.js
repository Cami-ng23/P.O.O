/*
    ...
    Muestra con alert() la frase unida por espacios.FF
*/

function construirFrase() {
    let frase = ["programar", "es "];
    frase.push("increible")
    frase.push("borrar")
    let eliminada = frase.pop("borrar")
    alert(`El ${frase.join(" ")}`)
...
function espera() {
    ...
    let nombre = [alumnos.shift("luis")];
    alert(`entro ${nombre} y la fila ahora es : ${alumnos.join(" ")}.`)
}
...
function consultorio() {
    ...
    alert(`quedan ${pacientes.length} pacientes y el siguiente es ${pacientes[[1]]}`)
}
